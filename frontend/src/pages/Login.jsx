...
import { LogIn, Sparkles, Mail, CheckCircle2, ShieldCheck, X, UserCheck, ArrowRight } from 'lucide-react';
...
const Login = () => {
  ...
  const handleOAuthAccountSelect = async (chosenEmail) => {
    ...
    setSuccessNotice(`Signing in as ${targetEmail}...`);
    const res = await socialLogin(provider, targetEmail);
    if (res && res.success) {
      setSuccessNotice(`🎉 Successfully signed in as ${targetEmail}! Redirecting...`);
      setTimeout(() => {
        navigate('/');
      }, 600);
    }
